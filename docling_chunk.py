...
def create_db_connection():
    ...
    cur.execute(
        "CREATE INDEX IF NOT EXISTS items_doc_name_idx ON items (doc_name);"
    )
    cur.execute(
        "CREATE INDEX IF NOT EXISTS items_chunk_hash_idx ON items (chunk_hash);"
    )
...
def main():
    ...
    for i, item in enumerate(generate_chunks("scratch/", chunker), start=1):
        print(f"Processing chunk #{i} "
              f"(doc={item['doc_name']}, doc_chunk={item['doc_chunk_index']})...\n")

        emb = get_embedding(item["chunk_text"])
        print(f"Chunk #{i} embedded. Now inserting into DB...\n")

        insert_chunk_and_embedding_to_db(item, emb, cur, conn)
        print(f"Chunk #{i} inserted into DB.\n")
