from typing import List
from sentence_transformers import SentenceTransformer
from .models import EMBEDDING_MODELS

_model_cache = {}

def get_embeddings(texts: List[str], model_key: str = "mpnet", is_query: bool = False):
    global _model_cache

    if model_key not in EMBEDDING_MODELS:
        raise ValueError(f"Unknown model: {model_key}. Choose from: {list(EMBEDDING_MODELS.keys())}")

    config = EMBEDDING_MODELS[model_key]

    # MedCPT uses separate encoders for documents vs queries
    if is_query and "query_model_name" in config:
        model_name = config["query_model_name"]
        cache_key = f"{model_key}_query"
    else:
        model_name = config["model_name"]
        cache_key = model_key

    if cache_key not in _model_cache:
        print(f"Loading {model_name}...")
        _model_cache[cache_key] = SentenceTransformer(model_name)

    model = _model_cache[cache_key]
    return model.encode(texts)