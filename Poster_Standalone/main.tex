 ...
 \vskip0.35cm
  \begin{columns}[c,totalwidth=\textwidth]
   \begin{column}{0.16\textwidth}
     \centering
     \includegraphics[height=4.2cm]{trinityLogo.png}
   \end{column}
   \begin{column}{0.68\textwidth}
    \centering
    {\color{PathwaysBurgundy}\fontsize{80pt}{86pt}\selectfont\textbf{\inserttitle}\\[0.35ex]}
    {\color{PathwaysGreen}\Large\itshape Intelligent document search for pediatric clinical decision support\\[0.9ex]}
    {\color{PathwaysGray}\large \insertauthor\\[0.5ex]}
    {\color{PathwaysGray}\normalsize \insertinstitute}
   \end{column}
   \begin{column}{0.16\textwidth}
     \centering
     \includegraphics[width=0.92\linewidth]{pathways_assets/ct_childrens_logo.png}
   \end{column}
  \end{columns}
 \vspace{0.18in}
 \centering
...
\author{Kwaku Agyapong \, \textperiodcentered \, Kitae Hwang}
...
\begin{block}{Monitoring and Future Improvements}
\normalsize
\centering
\includegraphics[width=0.97\linewidth,height=0.13\paperheight,keepaspectratio]{pathways_assets/supabase_table_crop.png}
\figcaption{Supabase session table: per-query timestamps, user ids, pathway ids, latencies.}

\vspace{0.6em}
\justifying
\begin{itemize}
\item Usage dashboard for session stats, response times, and retrieval behavior.
\item Response feedback so clinicians can grade answer quality.
\item Lighter preprocessing; continued prompt iteration.
\end{itemize}
\end{block}
...
\normalsize
\centering
\phasebox{PathwaysGreen}{Clinical alignment}{communication with CT Children's}
\hfill
\phasebox{PathwaysBurgundy}{Prior art}{learning from Seattle and Arizona}
\hfill
\phasebox{PathwaysPurple}{Impact}{building something that matters}
