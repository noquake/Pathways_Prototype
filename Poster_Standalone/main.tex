...
\setbeamertemplate{background canvas}{%
  \begin{tikzpicture}[remember picture,overlay]
    \fill[PathwaysCream] (current page.south west) rectangle (current page.north east);
  \end{tikzpicture}%
}
...
\setbeamertemplate{headline}{
 \leavevmode
 \vspace*{\dimexpr1cm+20pt\relax}
  \begin{columns}[c,totalwidth=\textwidth]
   \begin{column}{0.16\textwidth}
     \centering
     \includegraphics[height=5.1cm]{trinityLogo.png}
   \end{column}
...
% section titles). Keep the title-rule gap tight and place breathing room
% between sections instead.
\setbeamertemplate{block begin}
{
  \par\vskip1.4ex
  \begin{beamercolorbox}[colsep*=0ex,dp={0.5ex},leftskip=0pt]{block title}
    \usebeamerfont{block title}\Large\textbf{\insertblocktitle}\par
    \vspace{-0.7\baselineskip}\vspace{2pt}
...
\author{Kwaku Agyapong \textquoteright 26 \, \textperiodcentered \, Ki Tae Hwang \textquoteright 26}
...
\centering
\includegraphics[width=0.72\linewidth,height=0.19\paperheight,keepaspectratio]{pathways_assets/clinical_pathway_flowchart_bw.png}\\[0.15em]
